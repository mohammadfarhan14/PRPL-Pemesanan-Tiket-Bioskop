...
<head>
	...
	<?php 
		...
		$result = mysql_query("SELECT film.nama_film, jadwal.jam_mulai,jadwal.harga FROM film,jadwal WHERE jadwal.kode_jadwal=$kode_jadwal and film.kode_film=$kode_film");
		...
		for ($i=0; $i <sizeof($id_kursi) ; $i++) { 
			$query = mysql_query("SELECT * FROM kursi WHERE id_kursi =$id_kursi[$i]");
