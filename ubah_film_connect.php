<?php
	...
	$i = 1;
	$result = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM film");
	while ($data = mysqli_fetch_array($result)){
		if($img_file[$i]){
			...
			$update = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE film SET nama_film='$film[$i]',tgl_rilis='$tgl_rilis[$i]',poster_film='$poster_film' WHERE kode_film ='$i'");
			$i++;
	}
	for ($i=1;$i<3;$i++){
		$result1 = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM theater$i");
		while ($data1 = mysqli_fetch_array($result1)){
			$no_kursi =$data1['id_kursi'];
			$update_kursi = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE theater$i SET jadwal_1 = 'tersedia',jadwal_2 = 'tersedia',jadwal_3 = 'tersedia',`jadwal_4`='tersedia' WHERE id_kursi='$no_kursi'");
		}
	}
	if ($upload_status=1){
