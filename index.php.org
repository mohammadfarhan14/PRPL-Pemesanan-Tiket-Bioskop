...
<head>
	...
	<?php
		...
		$resultjadwal = mysql_query("SELECT * FROM jadwal");
		$countjadwal = 1;
		while($datajadwal = mysql_fetch_array($resultjadwal)){
		...
		$resulttheater = mysql_query("SELECT * FROM theater");
		$counttheater = 1;
		while($datatheater = mysql_fetch_array($resulttheater)){
		    	...
		    	<div class="title">
		    		JADWAL FILM <?php echo date('d')." ".date('m')." ".date('Y');?>
		 				...
		 				<form action="pesan.php" method="POST">	
			    			...
			    			$queryfilm = mysql_query("SELECT * FROM film WHERE akhir_tayang > NOW()");
			    			while ($datafilm = mysql_fetch_array($queryfilm)){
			    				$kode_film[$countfilm] = $datafilm['kode_film'];
								$nama_film[$countfilm] = $datafilm['nama_film'];
								$poster_film[$countfilm] = $datafilm['poster_film'];
								$resultfilm = mysql_query("SELECT theater.nama_theater,jadwal.jam_mulai,penayangan.id_theater,penayangan.id_jadwal,penayangan.id_film from penayangan left join jadwal on jadwal.kode_jadwal=penayangan.id_jadwal left join theater on theater.no_theater=penayangan.id_theater where penayangan.id_film=$kode_film[$countfilm] GROUP BY penayangan.id_theater,penayangan.id_jadwal order by theater.nama_theater") ;
								while ($datafilm=mysql_fetch_array($resultfilm)) {
