		    ...
		    <div class="white-box">
				...
				<table class="table-laporan" align="center">
					...
					<tbody>
						...
						$result= mysql_query("SELECT DISTINCT theater.nama_theater,jadwal.jam_mulai,film.nama_film,theater.kapasitas FROM theater,jadwal,film,transaksi WHERE theater.no_theater=transaksi.no_theater AND jadwal.kode_jadwal=transaksi.kode_jadwal AND film.nama_film=transaksi.nama_film  AND film.akhir_tayang > NOW() ORDER BY theater.nama_theater,jadwal.jam_mulai");
						$result1= mysql_query("SELECT DISTINCT theater.nama_theater,jadwal.jam_mulai,theater.kapasitas,film.nama_film, count(transaksi.id_transaksi) FROM theater JOIN jadwal  JOIN FILM  left JOIN transaksi on transaksi.no_theater=theater.no_theater and  transaksi.kode_jadwal=jadwal.kode_jadwal and transaksi.nama_film=film.nama_film and transaksi.waktu_transaksi like '2017-05-07%' where film.akhir_tayang > NOW() ORDER BY theater.nama_theater");
						...
						<tr>
							...
							<td>
							<?php 
							$result1= mysql_query("SELECT COUNT(*) FROM transaksi where no_theater=$i AND kode_jadwal=$j AND waktu_transaksi LIKE '$tanggal%'");
